...
// **************************************************************
// Check to make sure admin is logged in, otherwise, show login form
...
function showPlayerEditForm() 
{
	...
	$grp = $DB->select("SELECT id, name FROM player WHERE permban=0 ORDER BY name");
	foreach($grp as $d)
	{
		$playerlist2 .= "<option value='".$d['id']."'>".$d['name']."</option>";
	}
?>

	<div class="content-head">
		<div class="desc-title">Player Editor</div>
		<div class="description">
		<i>Description:</i> This is a custom made area where you can edit players rank, assign awards, and assign teams.
		</div>
	</div><br />
	<div class="content" align="center">
    	<form method="POST" action="index.php" name="adminform">
		<input type="hidden" name="action" value="process">
		<input type="hidden" name="task" value="changerank">
		<table border="0" width="40%" style="border: 2px solid #808080;">
			<tr>
				<td colspan="2" align="center" class="form-head">Rank Editor</td>
			</tr>
			<tr>
				<td width="40%">Player Name:</td>
                <td width="60%"><select name="player"><?php echo $playerlist2; ?></select>
                </td>
			</tr>
			<tr>
				<td width="40%">Set Rank:</td>
                <td width="60%"><select name="selected_rank">
            ...
            <tr>
            	<td align="center" colspan="2"><button name="change_rank" class="button" type="submit">Change Rank</button></td>
            </tr>
		</table>
        </form>
	</div>
<?php
// END showPlayerEditForm()
}

//	************************************************************	
// Main Config Edit Form
	...
	<div class="content">
		...
		<table border="0" width="95%" style="border: 2px solid #808080;">
			...
			<tr>
				...
				<input type="text" name="cfg__db_port" size="20" tabindex="<?php echo $i++; ?>" class="inputbox" value="<?php echo $cfg->get('db_port'); ?>" /></td>
...
function showMergePlayersForm() 
{
	...
	// Build Player List
	$player_list = "";
	$query = "SELECT id, name FROM player WHERE ip <> '127.0.0.1' ORDER BY name;";
			...
			if(mysql_num_rows($result)) 
			{
				while($row = mysql_fetch_array($result)) 
				{
					?>
						...
						<tr>
							<td colspan="10" align="right" class="form-pagenav">
				<?php
				} 
